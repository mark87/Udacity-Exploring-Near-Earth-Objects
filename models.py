...
class NearEarthObject:
    ...
    def __init__(self, designation, name, diameter = float('nan'), hazardous = False):
        ...
        '''Parameters:
        ---------------
        designation: str - if empty then None
        name: str - if empty then None
        diameter: float - if empty then float('nan')
        hazardous: Boolean - Return False or True based on 'N' or 'Y' in data file
    ...
    def __str__(self):
        ...
        return f"A NearEarthObject {self.fullname} has a diameter of " \
               f"{self.diameter:.3f} km and {self.hazardous} potentially " \
               f"hazardous."
...
class CloseApproach:
    ...
    '''Attributes:
        ---------------
        _designation: str
        time: Use the cd_to_datetime function for this attribute.
        distance: float - if empty then float('nan')
        velocity: float - if empty then float('nan')
        neo: None by default
        '''

    def __init__(self, designation, time, distance=float('nan'),
                 velocity=float('nan')):
    ...
    def __str__(self):
        ...
        return f"A CloseApproach at {self.time_str} approaches Earth at a " \
